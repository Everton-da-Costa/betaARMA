...
> `install.packages(c("ggplot2", "gridExtra", "zoo", "forecast", "scales"))`
...
knitr::kable(
  ...
  Brasília — subsample (February 2006 to August 2020)."
)
```
...
no serial correlation (Scher, Cribari-Neto, Pumi, & Bayer, 2020), 
indicating that the lag structure adequately captures 
the dynamics of the series.



```{r diagnostics_pcmle, fig.height=7.0, fig.cap="Diagnostic plots for the PCMLE fit: AR=(10,18), MA=(1,11,13). ACF and PACF spikes lie within the confidence bands and portmanteau test p-values indicate no evidence of residual serial correlation."}
...
A well-specified model should leave only white-noise residuals. The panels
below examine serial dependence and the overall distribution of the
residuals.
...
```{r diagnostics_hist, fig.height=3.5, fig.cap="Residual distribution for the reduced CMLE model."}
plot(fit_cmle_reduced, which = "hist")
```

All ACF and PACF spikes lie within the confidence bands, and the Ljung-Box
and Monti p-values remain consistently above 0.05 across evaluated lags,
indicating no evidence of residual serial correlation.
...
```{r forecast_setup}
n_test  <- length(y_test)
dates   <- as.Date(as.yearmon(time(y_test)))
months  <- tools::toTitleCase(format(dates, "%b"))
...
Standard CMLE failed to converge for the $\beta\text{ARMA}$ specification 
with $\text{AR} = (10, 18)$, $\text{MA} = (1, 11, 13)$
specification. Applying ridge penalization (PCMLE) restored convergence and
...
$\theta_{11}$ was not statistically significant, motivating the reduction to
the parsimonious $\beta\text{ARMA}$ specification with 
$\text{AR} = (10, 18)$, $\text{MA} = (1, 13)$ specification. The reduced
